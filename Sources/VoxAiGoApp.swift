...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func showLanguageNotification(language: SpeechLanguage) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + self.hudAutoCloseDelay) { [weak self, weak notificationWindow] in
    ...
    /// Auto-close delay for HUD notifications. Returns 30s when dev freeze is active, 2s otherwise.
    private var hudAutoCloseDelay: TimeInterval {
        SubscriptionManager.shared.devFreezeHUDs ? 30.0 : 2.0
    }
    ...
    func setupObservers() {
        ...
        // Dev HUD preview triggers (for screenshots)
        NotificationCenter.default.addObserver(
            forName: .devPreviewLanguageHUD, object: nil, queue: .main
        ) { [weak self] _ in
            self?.showLanguageNotification(language: SettingsManager.shared.outputLanguage)
        }
        NotificationCenter.default.addObserver(
            forName: .devPreviewModeHUD, object: nil, queue: .main
        ) { [weak self] _ in
            self?.showModeNotification(mode: SettingsManager.shared.selectedMode)
        }
        NotificationCenter.default.addObserver(
            forName: .devPreviewWakeWordHUD, object: nil, queue: .main
        ) { [weak self] _ in
            self?.showWakeWordNotification(label: SettingsManager.shared.selectedMode.localizedName, icon: SettingsManager.shared.selectedMode.icon)
        }
        NotificationCenter.default.addObserver(
            forName: .devPreviewPasteHUD, object: nil, queue: .main
        ) { [weak self] _ in
            self?.showPasteLastNotification(text: "Exemplo de texto colado para preview do HUD", mode: SettingsManager.shared.selectedMode)
        }
        NotificationCenter.default.addObserver(
            forName: .devPreviewNoHistoryHUD, object: nil, queue: .main
        ) { [weak self] _ in
            self?.showNoHistoryNotification()
        }
        NotificationCenter.default.addObserver(
            forName: .devPreviewRecordingHUD, object: nil, queue: .main
        ) { [weak self] _ in
            self?.window?.orderFrontRegardless()
        }
    }
    ...
    @objc func handleShowTrialExpired() {
        guard AuthManager.shared.isAuthenticated, !SubscriptionManager.shared.isPro else { return }
        DispatchQueue.main.async { [weak self] in
            guard let self = self else { return }
