#!/usr/bin/env python3
"""
Gerador de License Keys para VibeFlow
...
def generate_key():
    """Gera uma license key no formato VIBE-XXXX-XXXX-XXXX"""
...
def main():
    parser = argparse.ArgumentParser(description='Gerador de License Keys VibeFlow')
    ...
    with open(args.output, 'w') as f:
        f.write(f"# VibeFlow License Keys\n")
        f.write(f"# Gerado em: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"# Total: {len(keys)}\n")
        f.write(f"# Formato: VIBE-XXXX-XXXX-XXXX\n\n")
